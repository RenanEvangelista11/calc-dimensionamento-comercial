...
const fmtN = (n: number, d = 0) => n.toLocaleString("pt-BR", { minimumFractionDigits: d, maximumFractionDigits: d });
...
function NumberInput({ label, value, onChange, step = 1, min = 0 }: {
  ...
  const inc = () => onChange(String(Math.max(min, (parseFloat(value) || 0) + step)));
  const dec = () => onChange(String(Math.max(min, (parseFloat(value) || 0) - step)));
  return (
    <div>
      ...
      <div className="flex items-center rounded-lg overflow-hidden"
        ...
        <input
          type="number" inputMode="numeric" min={min} step={step}
          value={value} onChange={(e) => onChange(e.target.value)} placeholder="0"
          className="flex-1 bg-transparent px-3 py-3 text-sm text-white outline-none min-w-0"
          style={{ fontFamily: "'Fira Code', monospace" }}
        />
        <div className="flex flex-col border-l flex-shrink-0" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
          <button onClick={inc} className="px-3 py-1.5 text-xs leading-none hover:bg-white/10 cursor-pointer active:bg-white/20" style={{ color: "rgba(255,255,255,0.4)" }}>▲</button>
          <button onClick={dec} className="px-3 py-1.5 text-xs leading-none hover:bg-white/10 cursor-pointer active:bg-white/20 border-t" style={{ color: "rgba(255,255,255,0.4)", borderColor: "rgba(255,255,255,0.08)" }}>▼</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex rounded-lg overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
        {options.map((o, i) => (
          <button key={o.value} onClick={() => onChange(o.value)}
            className="flex-1 py-2.5 text-xs font-medium transition-all cursor-pointer active:opacity-80"
            style={{
  ...
  return (
    <div>
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full rounded-lg px-3 py-3 text-xs outline-none cursor-pointer"
        style={{
          background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)",
          color: "rgba(255,255,255,0.8)", appearance: "none",
...
function InputsPanel({
  ...
  nivelCloser: CloserNivel; setNivelCloser: (v: CloserNivel) => void;
  nivelSDR: SDRNivel; setNivelSDR: (v: SDRNivel) => void;
  baseCalculo: BaseCalculo; setBaseCalculo: (v: BaseCalculo) => void;
  pEntradaStr: string; setPEntradaStr: (v: string) => void;
  hasData: boolean;
  c: { percComis: number; comisVenda: number; comisRate: number };
}) {
  return (
              ...
              <div className="flex gap-1">
                {b.vals.map((v, i) => (
                  <span key={v} className="flex-1 text-center text-xs rounded py-0.5"
                    style={{
                      background: i === 0 ? "rgba(16,185,129,0.12)" : i === 1 ? "rgba(234,179,8,0.1)" : "rgba(239,68,68,0.1)",
                      color: i === 0 ? "#34d399" : i === 1 ? "#fbbf24" : "#f87171",
                    }}>{v}</span>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
      <div className="px-5 pt-5 pb-4">
        <p className="text-xs font-semibold mb-4" style={{ color: "rgba(255,255,255,0.28)", letterSpacing: "0.12em" }}>{title}</p>
...
function SectionDivider({ title }: { title: string }) {
  return (
    <div className="flex items-center gap-3 my-5">
      ...
      <p className="text-xs font-semibold" style={{ color: "rgba(255,255,255,0.22)", letterSpacing: "0.1em" }}>{title}</p>
      <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.06)" }} />
    </div>
  );
}
...
function FunnelBar({ label, sublabel, mainValue, metrics, color, bgColor, alert }: {
  ...
  return (
    <div className="relative rounded-xl overflow-hidden w-full"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pl-4 pr-4 pt-3.5 pb-3.5 gap-3 sm:gap-0">
        <div>
          <p className="text-xs font-semibold" style={{ color: `${color}bb`, letterSpacing: "0.08em" }}>{label}</p>
          {sublabel && <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>{sublabel}</p>}
          <p className="text-xl sm:text-2xl font-bold mt-1 text-white" style={{ fontFamily: "'Fira Code', monospace" }}>{mainValue}</p>
        </div>
          ...
          {metrics.map((m) => (
            <div key={m.label}>
              ...
              <p className="text-sm font-semibold mt-0.5"
                style={{ color: m.accent ? color : "rgba(255,255,255,0.7)", fontFamily: "'Fira Code', monospace" }}>
                {m.value}
              </p>
            </div>
          ))}
      ...
      {alert && (
        <div className="mx-3 mb-3 px-3 py-1.5 rounded-lg text-xs"
          style={{
...
export default function Home() {
  ...
  const insights = useMemo(() => {
    ...
    const maior  = pFolha >= pTraf && pFolha >= pComis ? "folha" : pTraf >= pComis ? "trafego" : "comissao";

    if (maior === "folha") {
      const redPJ  = regime === "clt" ? c.folha - c.folha / 1.5 : 0;
      const redRem = modalidade === "presencial" ? c.folha - c.folha / 1.25 : 0;
      ...
      if (regime === "clt" && modalidade === "presencial")
        body += ` Migrar para PJ remoto reduziria ~${fmt(redPJ + redRem)}/mês.`;
      else if (regime === "clt")
        body += ` Migrar para PJ economizaria ~${fmt(redPJ)}/mês na folha.`;
      else if (modalidade === "presencial")
        body += ` Remoto economizaria ~${fmt(redRem)}/mês.`;
      else if (nivelCloser === "lider")
        body += ` Closer Líder custa ${fmt(c.custoCloser)}/mês. Um Sênior entregaria 6 calls/dia por ~${fmt(CLOSER_DATA.senior.fixo * REGIME_MULT[regime] * MODAL_MULT[modalidade])}/mês.`;
      else if (c.numClosers >= 2)
        body += ` Com ${c.numClosers} closers, melhore a taxa de conversão antes de contratar mais.`;
      else
        body += ` Avalie regime e modalidade para reduzir o custo fixo.`;
    ...
    if (maior === "trafego") {
      ...
      if (c.vConversao > 0 && c.vConversao < 0.20) {
        ...
        body += ` CPL de ${fmt(c.vCpl)} está elevado para um ticket de ${fmt(c.vTicket)}. Testar canais orgânicos pode reduzir o CPL sem mexer no time.`;
      } else {
        body += ` Volume de leads alto (${fmtN(c.leads, 0)}/mês). Melhorar a qualificação reduz o volume sem perder vendas.`;
      }
    ...
    if (maior === "comissao") {
      ...
      if (c.comisRate > 0.05) {
        body += ` ${pct(c.comisRate)} supera o limite de 5% do ticket.`;
        if (baseCalculo === "cheio") {
          const comisEnt = c.vTicket * 0.4 * c.percComis * c.vendas;
          body += ` Mudar para entrada de caixa (40%) reduziria para ${fmt(comisEnt)}/mês — economia de ${fmt(c.comisTotal - comisEnt)}.`;
        } else {
          body += ` Revise o percentual para ficar abaixo de 5% do valor cheio.`;
        }
      } else if (baseCalculo === "cheio") {
        body += ` Calcular sobre entrada de caixa melhoraria o fluxo mensal da empresa.`;
      } else {
        body += ` Avalie se o percentual está alinhado com a margem do produto.`;
      }
      list.push({ type: tipo, tag: "GARGALO PRINCIPAL", title: "Comissão é o maior custo", body });
    }

    if (maior !== "trafego" && c.vConversao > 0 && c.vConversao < 0.15)
      list.push({ type: "warn", tag: "CONVERSÃO", title: "Taxa de conversão baixa",
        body: `Conversão de ${pct(c.vConversao)} exige ${fmtN(c.leads, 0)} leads/mês. Cada +5% reduz o volume proporcional de leads e tráfego.` });

    if (maior !== "comissao" && c.comisRate > 0.05)
      list.push({ type: "error", tag: "COMISSÃO", title: "Comissão acima do limite",
        body: `${pct(c.comisRate)} do ticket vai para comissão. ${baseCalculo === "cheio" ? "Mude para base de entrada de caixa para reduzir o desembolso mensal." : "Revise o percentual aplicado."}` });

    if (c.percCusto <= 0.35 && c.percCusto > 0)
      list.push({ type: "info", tag: "SAÚDE", title: "Estrutura dentro do limite",
        body: `Custo comercial de ${pct(c.percCusto)} abaixo dos 35%. O maior peso é ${maior === "folha" ? "a folha" : maior === "trafego" ? "o tráfego" : "a comissão"} — monitore se a meta crescer.` });
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#0b0d14", fontFamily: "var(--font-geist), -apple-system, sans-serif" }}>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          {/* Panel */}
          <div className="relative ml-auto w-full max-w-sm h-full overflow-y-auto flex flex-col"
            style={{ background: "#0f1117", borderLeft: "1px solid rgba(255,255,255,0.08)" }}>
            <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0"
              style={{ borderColor: "rgba(255,255,255,0.06)" }}>
              <p className="text-sm font-semibold text-white">Configurar Inputs</p>
              <button onClick={() => setDrawerOpen(false)}
                className="w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer hover:bg-white/10 active:bg-white/20"
            ...
            <div className="flex-1 overflow-y-auto">
              <InputsPanel {...inputProps} />
            </div>
            {hasData && (
              <div className="flex-shrink-0 px-5 py-4 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                <button onClick={() => setDrawerOpen(false)}
                  className="w-full py-3 rounded-xl text-sm font-semibold cursor-pointer transition-opacity active:opacity-80"
      ...
      <div className="flex" style={{ minHeight: "100dvh" }}>
        ...
        {/* ── DESKTOP SIDEBAR ───────────────────────────────────────────────── */}
        <aside className="hidden lg:flex w-72 xl:w-80 flex-shrink-0 flex-col"
          style={{ background: "#0f1117", borderRight: "1px solid rgba(255,255,255,0.06)", position: "sticky", top: 0, maxHeight: "100dvh", overflowY: "auto" }}>
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          {/* Top bar */}
          <div className="sticky top-0 z-10 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 border-b flex-shrink-0"
            style={{ background: "rgba(11,13,20,0.95)", backdropFilter: "blur(12px)", borderColor: "rgba(255,255,255,0.06)" }}>
            <div className="min-w-0">
              ...
              <p className="text-xs mt-0.5 truncate" style={{ color: "rgba(255,255,255,0.32)" }}>
                {hasData
                  ? `Meta ${fmt(c.vMeta)} · Ticket ${fmt(c.vTicket)}`
                  : "Configure os inputs para ver o fluxo"}
              </p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              {hasData && (
                <div className="hidden sm:flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs"
                  style={{
                  ...
                  {pct(c.percCusto)} — {isHealthy ? "Saudável" : "Atenção"}
                </div>
              )}
              {/* Mobile inputs button */}
              <button onClick={() => setDrawerOpen(true)}
                className="lg:hidden flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium cursor-pointer active:opacity-70"
          ...
          {/* Mobile health badge */}
          {hasData && (
            <div className="sm:hidden mx-4 mt-3 flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs"
              style={{
                background: isHealthy ? "rgba(16,185,129,0.08)" : "rgba(239,68,68,0.08)",
                border: `1px solid ${isHealthy ? "rgba(16,185,129,0.18)" : "rgba(239,68,68,0.18)"}`,
                color: isHealthy ? "#34d399" : "#f87171",
          ...
          {/* Funnel */}
          <div className="flex-1 px-4 sm:px-6 py-4 sm:py-6 max-w-3xl w-full">
            {!hasData ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-sm mb-5" style={{ color: "rgba(255,255,255,0.35)" }}>
                  Preencha a meta de faturamento e o ticket médio para ver o fluxo completo.
                </p>
                <button onClick={() => setDrawerOpen(true)}
                  className="lg:hidden flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold cursor-pointer active:opacity-80"
                  style={{ background: "linear-gradient(135deg, #6366f1, #a78bfa)", color: "white" }}>
                  Preencher inputs
                </button>
              </div>
              ...
              <>
                ...
                <FunnelBar label="META DE FATURAMENTO" sublabel="Receita bruta alvo no mês"
                  mainValue={fmt(c.vMeta)} color="#6366f1" bgColor="rgba(99,102,241,0.07)"
                  metrics={[{ label: "Ticket Médio", value: fmt(c.vTicket) }, { label: "Período", value: "Mensal" }]} />

                <FunnelArrow label="÷ Ticket" rate={fmt(c.vTicket)} />

                <FunnelBar label="VENDAS NECESSÁRIAS" sublabel="Contratos fechados no mês"
                  mainValue={`${fmtN(c.vendas)} vendas`} color="#3b82f6" bgColor="rgba(59,130,246,0.07)"
                  metrics={[
                ...
                <FunnelBar label="CALLS NECESSÁRIAS" sublabel="Reuniões de vendas agendadas"
                  mainValue={`${fmtN(c.calls)} calls`} color="#0ea5e9" bgColor="rgba(14,165,233,0.07)"
                  metrics={[
                ...
                <FunnelBar label="INVESTIMENTO EM TRÁFEGO" sublabel="Custo de mídia estimado"
                  mainValue={fmt(c.trafego)} color="#10b981" bgColor="rgba(16,185,129,0.07)"
                  metrics={[
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <FunnelBar label="CLOSERS" sublabel={CLOSER_DATA[nivelCloser].label}
                    ...
                    metrics={[
                      ...
                      { label: "Cap./mês",  value: String(c.capCloser) },
                    ]} />
                  <FunnelBar label="SDRs" sublabel={SDR_DATA[nivelSDR].label}
                ...
                <FunnelBar label="FOLHA SALARIAL"
                  sublabel={`${regime.toUpperCase()} · ${modalidade === "presencial" ? "Presencial" : "Remoto"}`}
                  mainValue={fmt(c.folha)} color="#8b5cf6" bgColor="rgba(139,92,246,0.07)"
                  metrics={[
                ...
                <div className="rounded-2xl overflow-hidden"
                  ...
                  {/* Numbers — stack on mobile */}
                  <div className="px-4 sm:px-6 pt-5 pb-4 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div>
                      <p className="text-xs font-semibold" style={{ color: "rgba(255,255,255,0.35)", letterSpacing: "0.1em" }}>CUSTO COMERCIAL TOTAL</p>
                      <p className="text-3xl sm:text-4xl font-bold mt-2 text-white" style={{ fontFamily: "'Fira Code', monospace" }}>{fmt(c.custoTotal)}</p>
                    ...
                    <div className="sm:text-right">
                      ...
                      <p className="text-3xl sm:text-4xl font-bold mt-1" style={{
                        fontFamily: "'Fira Code', monospace",
                        color: c.percCusto > 0.35 ? "#f87171" : c.percCusto > 0.28 ? "#fbbf24" : "#34d399",
                      }}>{pct(c.percCusto)}</p>
                      <p className="text-xs mt-1 font-medium" style={{ color: c.percCusto > 0.35 ? "#f87171" : "#34d399" }}>
                  ...
                  {/* Breakdown bar */}
                  {c.custoTotal > 0 && (
                    <div className="mx-4 sm:mx-5 mb-4 rounded-lg overflow-hidden flex h-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 mx-4 sm:mx-5 mb-5">
                    {[
                      { label: "Folha",     value: fmt(c.folha),      share: c.custoTotal > 0 ? pct(c.folha / c.custoTotal) : "—",      color: "#8b5cf6" },
                      ...
                      <div key={item.label} className="rounded-xl p-2.5 sm:p-3"
                        style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: item.color }} />
                          <span className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>{item.label}</span>
                        </div>
                        <p className="text-xs sm:text-sm font-bold text-white leading-tight" style={{ fontFamily: "'Fira Code', monospace" }}>{item.value}</p>
                  ...
                  {/* Insights */}
                  {insights.length > 0 && (
                    <div className="mx-4 sm:mx-5 mb-5 space-y-2">
                      {insights.map((ins, i) => {
                        const col = {
                        ...
                        return (
                          <div key={i} className="rounded-xl overflow-hidden" style={{ border: `1px solid ${col.border}` }}>
                            <div className="px-3 py-1.5" style={{ background: col.bg }}>
                              <span className="text-xs font-bold tracking-widest" style={{ color: col.tag }}>{ins.tag}</span>
                            </div>
                            <div className="px-4 py-3" style={{ background: "rgba(255,255,255,0.02)" }}>
                              ...
                              <p className="text-xs leading-relaxed" style={{ color: col.text }}>{ins.body}</p>
                            </div>
                          </div>
                        );
                      })}
